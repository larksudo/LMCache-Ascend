...
#define SINGLE_LAYER_PAGED_KV_COPY_V2_SEPARATE_TYPE_DECLARE(TYPE, SLOTTYPE)                                     \
    extern "C" __global__ __aicore__ void single_layer_paged_kv_copy_v2_separate_##TYPE##_##SLOTTYPE(           \
        ...
        __gm__ uint8_t* vllmDsaKeyPtr, __gm__ uint8_t* slotMappingPtr,                                          \
        const int64_t keyBlockStride, const int64_t valueBlockStride, const int64_t dsaKeyBlockStride,          \
        const int64_t vllmKeyBufferSize, const int64_t vllmValueBufferSize, const int64_t vllmDsaKeyBufferSize, \
    ...
    {                                                                                                           \
        ...
        op.GetPolicy().Init(vllmKeyPtr, vllmValuePtr, vllmDsaKeyPtr,                                            \
                            keyBlockStride, valueBlockStride, dsaKeyBlockStride,                                \
                            vllmKeyBufferSize, vllmValueBufferSize, vllmDsaKeyBufferSize,                       \
                            numHeads, headDims, vHeadDims, dsaHeadDims, blockSize);                             \
        /* 2. Initialize Common parameters */                                                                   \
        op.InitCommon(lmcKeyValueCachePtr, slotMappingPtr, lmcTokenStride, lmcValueOffset,                      \
                      lmcDsaOffset, lmcBufferSize,                                                              \
                      maxTokensPerLoop, numHeads, headDims, vHeadDims, dsaHeadDims,                             \
...
#define SINGLE_LAYER_PAGED_KV_COPY_V2_SEPARATE_KERNEL_CALL(TYPE, SLOTTYPE)                                 \
    single_layer_paged_kv_copy_v2_separate_##TYPE##_##SLOTTYPE<<<blockDim, nullptr, stream>>>(             \
        lmcKeyValueCachePtr, vllmKeyPtr, vllmValuePtr, vllmDsaKeyPtr, slotMappingPtr,                      \
        keyBlockStride, valueBlockStride, dsaKeyBlockStride,                                               \
        vllmKeyBufferSize, vllmValueBufferSize, vllmDsaKeyBufferSize,                                      \
        lmcTokenStride, lmcValueOffset, lmcDsaOffset, lmcBufferSize,                                       \
        maxTokensPerLoop, numHeads, headDims, vHeadDims, dsaHeadDims,                                      \
        numTokens, blockSize, page2L, lmcTokensMajor);
...
void single_layer_paged_kernel_v2_separate(
    ...
    uint8_t* vllmKeyPtr, uint8_t* vllmValuePtr, uint8_t* vllmDsaKeyPtr, uint8_t* slotMappingPtr,
    const int64_t keyBlockStride, const int64_t valueBlockStride, const int64_t dsaKeyBlockStride,
    const int64_t vllmKeyBufferSize, const int64_t vllmValueBufferSize, const int64_t vllmDsaKeyBufferSize,
    const int64_t lmcTokenStride, const int64_t lmcValueOffset, const int64_t lmcDsaOffset,
    const int64_t lmcBufferSize, const int32_t maxTokensPerLoop, const int32_t numHeads,
    const int32_t headDims, const int32_t vHeadDims, const int32_t dsaHeadDims,
...
#define SINGLE_LAYER_PAGED_KERNEL_CALL_TYPE_SLOTTYPE_SEPARATE_DECLARE_HOST(TYPE)      \
...
SINGLE_LAYER_PAGED_KERNEL_CALL_TYPE_SLOTTYPE_SEPARATE_DECLARE_HOST(half);
SINGLE_LAYER_PAGED_KERNEL_CALL_TYPE_SLOTTYPE_SEPARATE_DECLARE_HOST(int8_t);
#if (ASCEND_AICORE_ARCH >= 220)
SINGLE_LAYER_PAGED_KERNEL_CALL_TYPE_SLOTTYPE_SEPARATE_DECLARE_HOST(bfloat16_t);
#endif
...
void dispatch_single_layer_kernel_v2_separate_on_slot_type(
        ...
        uint8_t* vllmDsaKeyPtr, uint8_t* slotMappingPtr,
        const int64_t keyBlockStride, const int64_t valueBlockStride, const int64_t dsaKeyBlockStride,
        const int64_t vllmKeyBufferSize, const int64_t vllmValueBufferSize, const int64_t vllmDsaKeyBufferSize,
        const int64_t lmcTokenStride, const int64_t lmcValueOffset, const int64_t lmcDsaOffset,
        const int64_t lmcBufferSize, const int32_t maxTokensPerLoop,
        const int32_t numHeads, const int32_t headDims, const int32_t vHeadDims, const int32_t dsaHeadDims,
        const int32_t numTokens, const int32_t blockSize, const bool page2L, const bool lmcTokensMajor)
{
    switch(slotType) {
        case kvcache_ops::AscendType::INT32:
            single_layer_paged_kernel_v2_separate<T, int32_t>(
                blockDim, stream, lmcKeyValueCachePtr, vllmKeyPtr, vllmValuePtr, vllmDsaKeyPtr, slotMappingPtr,
                keyBlockStride, valueBlockStride, dsaKeyBlockStride,
                vllmKeyBufferSize, vllmValueBufferSize, vllmDsaKeyBufferSize,
                lmcTokenStride, lmcValueOffset, lmcDsaOffset, lmcBufferSize, maxTokensPerLoop,
                numHeads, headDims, vHeadDims, dsaHeadDims, numTokens, blockSize, page2L, lmcTokensMajor
            );
            break;
...
extern void single_layer_kv_transfer_kernel_v2_separate(
    ...
    uint8_t* vllmValuePtr, uint8_t* vllmDsaKeyPtr, uint8_t* slotMappingPtr,
    const int64_t keyBlockStride, const int64_t valueBlockStride, const int64_t dsaKeyBlockStride,
    const int64_t vllmKeyBufferSize, const int64_t vllmValueBufferSize, const int64_t vllmDsaKeyBufferSize,
    const int64_t lmcTokenStride, const int64_t lmcValueOffset, const int64_t lmcDsaOffset,
    const int64_t lmcBufferSize,
    const int32_t maxTokensPerLoop, const int32_t numHeads, const int32_t headDims,
    const int32_t vHeadDims, const int32_t dsaHeadDims,
    ...
    const bool page2L, const bool lmcTokensMajor)
{
    switch(type) {
        case kvcache_ops::AscendType::FP16:
            dispatch_single_layer_kernel_v2_separate_on_slot_type<half>(slotType, blockDim, stream,
                lmcKeyValueCachePtr, vllmKeyPtr, vllmValuePtr, vllmDsaKeyPtr, slotMappingPtr,
                keyBlockStride, valueBlockStride, dsaKeyBlockStride,
                vllmKeyBufferSize, vllmValueBufferSize, vllmDsaKeyBufferSize,
                lmcTokenStride, lmcValueOffset, lmcDsaOffset, lmcBufferSize, maxTokensPerLoop,
        ...
        case kvcache_ops::AscendType::BF16:
            dispatch_single_layer_kernel_v2_separate_on_slot_type<bfloat16_t>(slotType, blockDim, stream,
                lmcKeyValueCachePtr, vllmKeyPtr, vllmValuePtr, vllmDsaKeyPtr, slotMappingPtr,
                keyBlockStride, valueBlockStride, dsaKeyBlockStride,
                vllmKeyBufferSize, vllmValueBufferSize, vllmDsaKeyBufferSize,
                lmcTokenStride, lmcValueOffset, lmcDsaOffset, lmcBufferSize, maxTokensPerLoop,
        ...
        case kvcache_ops::AscendType::INT8:
            dispatch_single_layer_kernel_v2_separate_on_slot_type<int8_t>(slotType, blockDim, stream,
                lmcKeyValueCachePtr, vllmKeyPtr, vllmValuePtr, vllmDsaKeyPtr, slotMappingPtr,
                keyBlockStride, valueBlockStride, dsaKeyBlockStride,
                vllmKeyBufferSize, vllmValueBufferSize, vllmDsaKeyBufferSize,
                lmcTokenStride, lmcValueOffset, lmcDsaOffset, lmcBufferSize, maxTokensPerLoop,
