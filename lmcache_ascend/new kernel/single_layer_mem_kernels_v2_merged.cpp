...
#define SINGLE_LAYER_PAGED_KV_COPY_V2_TYPE_DECLARE(TYPE, SLOTTYPE)                                              \
    ...
    {                                                                                                           \
        AscendC::TPipe pipe;                                                                                    \
        SingleLayerPagedKVCopyProcessor<TYPE, SLOTTYPE, MergedPolicy<TYPE>> op{};                                 \
        /* 1. Initialize Policy-specific parameters */                                                          \
        op.GetPolicy().Init(vllmKeyValuePtr, vllmBlockStride, vllmValueOffset, vllmBufferSize,                  \
                            numHeads, headDims, blockSize);                                                     \
        /* 2. Initialize Common parameters (merged: vHeadDims == headDims, no DSA) */                            \
        op.InitCommon(lmcKeyValueCachePtr, slotMappingPtr, lmcTokenStride, lmcValueOffset,                      \
                      0, lmcBufferSize,                                                                         \
                      maxTokensPerLoop, numHeads, headDims, headDims, 0,                                        \
...
#define SINGLE_LAYER_PAGED_KERNEL_CALL_TYPE_SLOTTYPE_DECLARE_HOST(TYPE)      \
...
SINGLE_LAYER_PAGED_KERNEL_CALL_TYPE_SLOTTYPE_DECLARE_HOST(half);
SINGLE_LAYER_PAGED_KERNEL_CALL_TYPE_SLOTTYPE_DECLARE_HOST(int8_t);
#if (ASCEND_AICORE_ARCH >= 220)
SINGLE_LAYER_PAGED_KERNEL_CALL_TYPE_SLOTTYPE_DECLARE_HOST(bfloat16_t);
#endif
