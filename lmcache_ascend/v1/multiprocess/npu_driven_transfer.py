...
def transfer_kv_per_object_group(
    ...
    """
    # The direct batch path below invokes the same multi-layer block operator
    # and is the correctness baseline.  The single-call object-group planner
    # is opt-in until its asynchronous completion contract is verified on
    # Ascend; otherwise a stalled plan monopolises the MP affinity worker.
    use_object_group_plan = os.getenv(
        "LMCACHE_ASCEND_ENABLE_OBJECT_GROUP_PLAN", "0"
    ).lower() in {"1", "true", "yes"}
    if use_object_group_plan and _HAS_NATIVE_OBJECT_GROUP_TRANSFER and not any(
    ...
    # Ascend cannot import the server process's ACL event handle.  Finish all
    # queued kernel and host-copy work before the MP handler replies; clients
    # then use the ZMQ response itself as the completion fence.
    torch.npu.synchronize(cache_context.device)
